...
class Messages extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    // When the message area is expanded, scroll to the bottom
    ...
    if(this.state.show) {
      return (
        <div style={{display: 'flex', flexDirection: 'column'}}>
          ...
          <div ref={this.messageAreaRef} style={{height: 500, width: 300, border: '1px solid #aaaaaa', backgroundColor: 'white', overflowY: 'auto', paddingRight: 20}}>
            {this.getMessages()}
          </div>
        </div>
      );
